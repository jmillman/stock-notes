...
export class ItemsFilter {
  ...
  getDateForCalendarControl() {
    return `${dateDBToCalendar(this.dateStart)} - ${dateDBToCalendar(
      this.dateEnd
    )}`;
  }
  ...
  filter(items) {
    ...
    if (this.itemTypeIds && this.itemTypeIds.length) {
      retItems = retItems.filter((item) =>
        this.itemTypeIds.includes(item.typeId)
      );
    }

    if (this.dateStart && this.dateEnd) {
      retItems = retItems.filter(
        (item) => item.date >= this.dateStart && item.date <= this.dateEnd
      );
