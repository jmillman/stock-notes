...
import { Message, Segment, Checkbox, Form, Button } from 'semantic-ui-react';
  ...
  return (
    <Form>
      ...
      <Form.Field key={'CreateTaskListAddButton'}>
        <input
          placeholder={
            'Create a new task list by typing a name here, select some tasks, then clicking add.'
          }
          value={name}
        ...
        return (
          <Segment key={itemType.id}>
            <Checkbox
              label={itemType.name}
              onClick={(e, data) =>
                handleClickCheckbox(itemType.id, data.checked)
              }
      ...
      <Form.Field key={'Buttons'}>
        {taskListToEditId ? (
          <Button positive onClick={handleClickEdit}>
            Edit
          </Button>
        ) : (
        ...
        <Button color="orange" onClick={resetForm}>
          Reset
        </Button>
        {taskListToEditId ? (
          <Button negative onClick={handleClickDelete} floated="right">
            Delete
          </Button>
        ) : null}
      </Form.Field>
