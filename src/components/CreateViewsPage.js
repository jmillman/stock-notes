...
function CreateViewsPage() {
  ...
  function handleSelectedView(viewId) {
    console.log(date); //here to get rid of lint
    setSelectedViewId(viewId);
    const item = state.views.filter((view) => {
  ...
  return (
    <Grid columns="12">
      <Grid.Column width={4}>
        <Container width="100%">
          ...
          {/* <DatesRangeInput
                        ...
                        /> */}
          <ItemTypeCheckboxes
            callback={handleSetSelectedItemTypes}
      ...
      <Grid.Column width={8}>
        {getData().map((item) => {
          return <CompletedItem item={item} key={item.id} />;
        })}
      </Grid.Column>
    </Grid>
  );
