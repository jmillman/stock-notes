...
function TasksPage() {
  ...
  function handleSelectTaskList(taskID) {
    if (taskID === ALL) {
      ...
      const taskList = state.taskLists.filter((taskList) => {
        return taskList.id === taskID;
      })[0];
      const itemsInList = taskList.notes.split(',');
      const itemTypes = itemsInList.reduce(
        (result, key) => ({ ...result, [key]: state.itemTypes[key] }),
        {}
      );
    ...
    <>
      <Grid columns="equal" padded>
        <Grid.Row key={'TaskPage'}>
          ...
          <Grid.Column width={1}>
            <Button icon onClick={handleClickToggleCalendar}>
              <Icon name="calendar" />
            </Button>
          </Grid.Column>
        </Grid.Row>
