...
import { Button, Input, Select, Label, Grid } from 'semantic-ui-react';
...
function AddItem(props) {
  ...
  const [selectedItemType, setSelectedItemType] = useState(
    props.itemType ? props.itemType.id : ''
  );
        ...
        <>
          ...
          <datalist id="itemTypesList">
            {itemTypes.map((itemType) => {
              return <option value={itemType.name} key={itemType.id} />;
            })}
          </datalist>
        </>
      );
  ...
  return (
    <>
      <Grid.Row key={`AddItem-${props.itemType.id}`}>
        <Grid.Column width={7}>{getSelectTypeChooser()}</Grid.Column>
        <Grid.Column width={5}>{itemFormChooser()}</Grid.Column>
        <Grid.Column width={1}>
          {saved === false ? (
