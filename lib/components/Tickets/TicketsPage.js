            ...
            <div className='col-8'>
              <div>
                {allTicketsData.length && allTicketsData.map((ticket) => {
                  return <span key={ticket.ticket + ticket.priceAndTime}><a href={'http://localhost:4444/createbet?ticket=' + ticket.ticket + '&priceAndTime=' + ticket.priceAndTime}>{ticket.ticket} {ticket.userId} {ticket.price} </a><br /><br /></span>;
                })}
              </div>
            </div>
          </div>
