...
class FootballGameToWinCreateBet extends React.Component {
  constructor(props, context) {
    super(props, context);
    this.state = {
      home: parseFloat(5,10).toFixed(2),
      away: parseFloat(5,10).toFixed(2)
    };
  }
  changePrice(step) {
    ...
    if(home + step < max && home + step > 0) {
      home = (home + step).toFixed(2);
      away = (max - home).toFixed(2);
  ...
  validateBet() {
    ...
    if(parseFloat(home) + parseFloat(away) != 10) {
      isValid = false;
      alert('The buy and sell should equal 10');
    }
