...
function* getDailyNotesSaga() {
  const response = yield effects.call(
    fetch,
    'http://127.0.0.1:5000/stock_notes?date=2021-03-23'
  );
...
function* lookupSymbolSaga(action) {
  yield effects.put(
    actions.lookupSymbolResponse({ status: 'Searching Symbol....' })
  );
  const response = yield effects.call(
    fetch,
    `http://127.0.0.1:5000/lookup_symbol?symbol=${action.symbol}`
  );
