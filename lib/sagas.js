...
function * getDailyNotesSaga () {
  const response = yield effects.call(fetch, 'http://localhost:5000/stock_notes?date=2021-03-23');
...
function * lookupSymbolSaga (action) {
  ...
  const response = yield effects.call(fetch, `http://localhost:5000/lookup_symbol?symbol=${action.symbol}`);
