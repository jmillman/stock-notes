...
function footballGameDataReducer(footballGameData = {}, action) {
  ...
  if (footballGameData == 'asdfasdf') return null;
...
function footballBetViewOptionsReducer(viewOptions = {}, action) {
  switch (action.type) {
    case constants.INITIALIZE_FOOTBALL_BET_OPTIONS: {
      ...
      let game = action.footballGameData.games.filter((game) => {
        return game.date == date;
      })[0];
      let betType = constants.BET_TYPE_FOOTBALL_TO_WIN;
      return Object.assign({}, viewOptions, { date, game, betType });
    }
...
function notesReducer(notesData = {}, action) {
  switch (action.type) {
    case constants.GET_DAILY_STOCK_NOTES_RESPONSE:
      return action.data;
    default:
      return notesData;
  }
...
function statusReducer(status = {}, action) {
  switch (action.type) {
    case constants.LOOKUP_SYMBOL_RESPONSE:
      return action.data;
    default:
      return status;
  }
...
function dateReducer(date = moment().format('YYYY-MM-DD'), action) {
  switch (action.type) {
